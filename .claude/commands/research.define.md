...
The text the user typed after `/research.define` in the triggering message **is** the research topic description. Assume you always have it available in this conversation even if `$ARGUMENTS` appears literally below. Do not ask the user to repeat it unless they provided an empty command.
...
2. **Check for existing branches before creating new one**:
   ...
   d. Run the script `.research/scripts/bash/create-new-research.sh --json "$ARGUMENTS"` with the calculated number and short-name:
      - Pass `--number N+1` and `--short-name "your-short-name"` along with the research description
      - Bash example: `.research/scripts/bash/create-new-research.sh --json "$ARGUMENTS" --json --number 5 --short-name "ai-healthcare" "Research AI trends in healthcare"`
      - PowerShell example: `.research/scripts/bash/create-new-research.sh --json "$ARGUMENTS" -Json -Number 5 -ShortName "ai-healthcare" "Research AI trends in healthcare"`
...
3. Load `.research/templates/research-definition-template.md` to understand required sections.
